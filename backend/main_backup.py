...
try:
    ...
    import openai
    OpenAI = None  # We'll handle this case below
...
# We're using a local model now, so we don't need the API clients
...
FAQ: Dict[str, Dict] = {
    ...
    "fir": {
        "answer": "You can file an FIR (First Information Report) at the nearest police station where the incident occurred.",
        "legal_reference": "Section 154 of the Criminal Procedure Code, 1973",
        "explanation": "An FIR is a written document prepared by police when they receive information about a cognizable offense. It's the first step in the criminal justice process.",
        "steps": [
            "Visit the police station with jurisdiction over the area where the incident occurred.",
            "Provide a detailed written complaint with all relevant facts, dates, names, and evidence.",
            "Ensure you get a signed copy of the FIR with the FIR number for future reference.",
            "If police refuse to file an FIR, you can approach the Superintendent of Police or file a complaint with the Magistrate under Section 156(3) CrPC."
        ]
    }
}
    ...
    for key in list(FAQ.keys()) + list(KEYWORD_ALIASES.keys()):
        if key in user_msg:
            ...
            if canonical in FAQ:
                ...
                return resp_obj
    # Retrieval-based fallback using local RAG pipeline
    rag_result = answer_query(user_msg)
    final_answer = rag_result.get("answer", "")
    if target_lang and target_lang.lower() not in {"english", "en"}:
        final_answer = translate_text(final_answer, target_lang)
